...
describe("[Challenge] Free Rider", function () {
   ...
   * @dev
   * Exploit overview:
   * The core issues lie in the private function _buyOne.
   * The owner of nft is paid after transfer of nft from buyer to seller so the payment goes to the buyer
   * and not the seller.
   * Also require(msg.value >= priceToPay, "Amount paid is not enough") this line checks for msg.value
   * to be greater than nftPrice but msg.value is same for all 6 transactions so instead of checking if
   * msg.value >= 6*nftPrice it ends up checking only if msg.value > nftPrice which is core design issue.
   * We can effectively buy any amount of Nfts while paying the price of only 1.
   * To gather the amount of funds required to pull of the attack we have to flashswap the required weth amount
   * from the uniswapv2 protocol and return it after the attack succeeds all in a single flashswap transaction.
   */
